...
module Board = 
    ...
    let EndgameMateKingPcSq loseKing (winKing : Position) = 
        let getsqs pos = 
            let d1 = pos|>Position.DistanceToNoDiag(Position.D4)
            let d2 = pos|>Position.DistanceToNoDiag(Position.D5)
            let d3 = pos|>Position.DistanceToNoDiag(Position.E4)
            let d4 = pos|>Position.DistanceToNoDiag(Position.E5)
            let minDist = [ d1; d2; d3; d4 ] |> List.min
            minDist * 50
        let kPcSq = Position.AllPositions|>Array.map getsqs
        PhasedScore.Create 0 (kPcSq.[int (loseKing)] - ((winKing|>Position.DistanceTo(loseKing)) * 25))
    ...
    let MhEmp = MhSet MoveEmpty Position.OUTOFBOUNDS CstlFlgs.All 0 0 0L 0L 0L Bitboard.Empty
    ...
    let BoardZob(bd : Brd) = 
        ...
        let zep = if bd.EnPassant|>Position.IsInBounds then Zobrist.Enpassant(bd.EnPassant) else 0L
    ...
    let MoveUndo(bd : Brd) = 
        ...
        let bd = 
            if moveUndoing |> IsCastle then 
                ...
                if moveUndoing|>MovingPlayer = Player.White && (moveUndoing|>To) = Position.G1 then 
                    bd |> PieceMove Position.F1 Position.H1
                elif moveUndoing|>MovingPlayer= Player.White && (moveUndoing|>To) = Position.C1 then 
                    bd |> PieceMove Position.D1 Position.A1
                elif moveUndoing|>MovingPlayer = Player.Black && (moveUndoing|>To) = Position.G8 then 
                    bd |> PieceMove Position.F8 Position.H8
                else bd |> PieceMove Position.D8 Position.A8
            else bd
        
        let bd = 
            if moveUndoing |> IsEnPassant then 
                let tofile = moveUndoing|>To|>Position.ToFile
                let enpassantRank = moveUndoing|>MovingPlayer|>Player.MyRank(Rank.Rank5)
                bd 
    ...
    let PieceInDirection (from : Position) (dir : Direction) (bd : Brd) = 
        let rec getpospc dist (pos : Position) pc = 
            if not (pos|>Position.IsInBounds) then pc, pos
            else 
                let npc = bd.PieceAt.[int (pos)]
                if npc <> Piece.EMPTY then npc, pos
                elif dir |> Direction.IsDirectionKnight then npc, pos
                else 
                    let npos = pos|>Position.PositionInDirection(dir)
                    getpospc (dist + 1) npos npc
        getpospc 1 (from|>Position.PositionInDirection(dir)) Piece.EMPTY
    ...
    let HistMove (movesAgo : int) (bd : Brd) = (List.nth bd.HistL (movesAgo-1)).Move
