...
module Attacks = 
    let Combinations(mask : Bitboard) = 
        ...
        let getc i = 
            ...
            |> List.mapi (fun b p -> 
                   ...
                   else p |> Position.ToBitboard)
            |> List.reduce (|||)
        [ 0..possCombs - 1 ] |> List.map getc
    
    let RookMaskCalc(position : Position) = 
        let rec getr d (p : Position) rv = 
            if p
               |> Position.PositionInDirection(d)
               = Position.OUTOFBOUNDS then rv
            else 
                let nrv = rv ||| (p |> Position.ToBitboard)
                getr d (p |> Position.PositionInDirection(d)) nrv
        Direction.AllDirectionsRook
        |> Array.map (fun d -> getr d (position |> Position.PositionInDirection(d)) Bitboard.Empty)
        |> Array.reduce (|||)
    
    let BishopMaskCalc(position : Position) = 
        let rec getr d (p : Position) rv = 
            if p
               |> Position.PositionInDirection(d)
               = Position.OUTOFBOUNDS then rv
    ...
    let RookAttacksCalc (position : Position) (blockers : Bitboard) = 
        let rec getr d p rv = 
            if p = Position.OUTOFBOUNDS then rv
            else 
                let nrv = rv ||| (p |> Position.ToBitboard)
                if blockers |> Bitboard.ContainsPos(p) then nrv
                else getr d (p |> Position.PositionInDirection(d)) nrv
        Direction.AllDirectionsRook
        |> Array.map (fun d -> getr d (position |> Position.PositionInDirection(d)) Bitboard.Empty)
        |> Array.reduce (|||)
    
    let BishopAttacksCalc (position : Position) (blockers : Bitboard) = 
        let rec getr d p rv = 
            if p = Position.OUTOFBOUNDS then rv
            else 
                let nrv = rv ||| (p |> Position.ToBitboard)
    ...
    let AttacksKn = 
        let getkn sq = 
            ...
            |> Seq.map (fun d -> 
                   ...
                   |> Position.ToBitboard)
            |> Seq.reduce (|||)
        Position.AllPositions |> Array.map getkn
    ...
    let AttacksP, AttacksPF = 
        let getp pr sq = 
            let board = 
                Bitboard.Empty 
                ||| (((sq |> Position.PositionInDirection(pr |> Player.MyNorth)) 
                      |> Position.PositionInDirection(Direction.DirE)) |> Position.ToBitboard) 
                ||| (((sq |> Position.PositionInDirection(pr |> Player.MyNorth)) 
                      |> Position.PositionInDirection(Direction.DirW)) |> Position.ToBitboard)
